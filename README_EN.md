...
flowchart LR
    ...
    G --> H["submission-condition inference"]
```
...
| `WAYB_WAYC_metadata_test.csv` | submission conditions, sample IDs, submission order | No |
...
Multiple controls are averaged per protein. A metric position is used only if both the treatment and matched control protein are observed. B1 is a local validation diagnostic and the source of P1 training deltas; submission predictions are generated by the condition model.
...
Generate and validate a submission prediction:
...
- Statistical priors are fit on training rows only.
- Input paths and sample_ID alignment are validated before a run; Git ignores competition CSVs, checkpoints, predictions, runs, reference PDFs, and temporary files.
